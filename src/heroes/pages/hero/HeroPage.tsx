...
import { NotFoundPage } from "../errors/404-not-found"
...
export const SuperheroProfile = () => {
    /* 
        Obtenemos el idSlug directamente de la URL para que esto funcione es necesario que en el router nosotros tengamos declarado
        una variable dinamica en el path ejemplo:

        path: 'heroes/:idSlug',

        Para que :idSlug funcione correctamente nosotros necesitamos utilizar el hook Navigate como se muestra a continuacion
        const handleClick = (slug: string) => {
            navigate(`/heroes/${slug}`)
        }

        de esta manera la libreria react-router identificara de manera automatica que el :idSlug debe ser remplazado por lo que 
        el metodo navigate esta utilizando como variable en este caso el slug.
    */
    ...
    useEffect(() => {
        if (isError) {
            const timeout = setTimeout(() => {
                setRedirect(true); // activa la redirección después de 5s
            }, 5000);
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="flex justify-between items-center py-2">
                                        ...
                                        <span className="font-semibold">
                                            {new Date().getFullYear() - Number.parseInt(superheroData?.firstAppearance ?? 'desconocido')} años
                                        </span>
                                    </div>
                                </CardContent>
                            </Card>
